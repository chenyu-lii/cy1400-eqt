...
def threshold_check(another_detection_folder, threshold_main = 0.3, threshold_p = 0.1, threshold_s = 0.1):
	...
	detection_folder_name = another_detection_folder
	...
	for csv_file in glob.glob("{}/*/*.csv".format(detection_folder_name)):
		...
		for c, p_arrival in enumerate(pick_date_times):
			...
			_st.trim(start_UTC_time + delta_t - 5, start_UTC_time + delta_t + 30)
...
for c, (t1, t2, t3) in enumerate(thresholds):
	threshold_check(detection_folders[c], t1, t2, t3)